...
import net.sourceforge.dynamicgmf.diagram.edit.helpers.EcoreModelBaseEditHelper;
 ...
 * @generated
 */
public class EcoreModelBaseItemSemanticEditPolicy extends SemanticEditPolicy {
	...
	protected Command getSemanticCommand(IEditCommandRequest request) {
		...
		if (semanticCommand != null) {
			...
			completedRequest.setParameter(
					EcoreModelBaseEditHelper.EDIT_POLICY_COMMAND, command);
		}
		if (elementType != null) {
	...
	public static class LinkConstraints {
		...
		public static boolean canCreateReference(EObject source, EObject target,
				EReference eReference) {
			if (source != null) {
				if (eReference.isMany()) {
					EList containings = (EList) source.eGet(eReference);
					if (containings.contains(target)) {
						return false;
					}
				} else {
					if (target != null && source.eGet(eReference) == target) {
						return false;
					}
				}
			}
			return canExistReference(source, target);
		}

		public static boolean canExistReference(EObject source, EObject target) {
			return true;
		}
	}
